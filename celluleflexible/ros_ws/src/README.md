...
Dans cet espace de travail, vous trouverez les dossiers générés par la compilation (`build`, `install`, `log`) ainsi que le dossier source (`src`) contenant les différents paquets.
...
* **`robots`** : Gère les 4 bras robotiques. Chaque bras est un nœud indépendant qui exécute les séquences de prise/dépose de pièces et gère les chronomètres de fabrication, tout en assurant l'animation visuelle (changement de couleur/opacité) dans CoppeliaSim.
...
* **`colcon build`** : Compile l'ensemble des paquets présents dans le dossier `src`.
...
Une fois CoppeliaSim lancé par l'étape 2, le code de `commande_locale` va lui-même déclencher (de manière automatisée) le lancement du niveau supérieur de la simulation, à savoir :

* L'appel de `launch_beta.launch.py` (du paquet `launcher`), qui va instancier et ouvrir des terminaux séparés pour :
* Le nœud `run` (Réseau de Pétri du paquet `commande`).
* Le nœud `run_Aiguillage` (gestion centralisée des aiguillages).
* Le nœud `ShuttleManager` (supervision des navettes).
* Le nœud `log_manager` (gestion des logs, voir section 4).
* Les nœuds `Robot1` à `Robot4` (contrôle indépendant de chaque bras).
