...
* Lancement du nœud `simulation` appartenant au package `commande_locale`.
...
* Déployer les bras robotiques `Robot1` et `Robot2` de manière inconditionnelle.
...
#### 3.2.1. Sécurités et conditions mises en place
...
3. Vérifiez que 7 fenêtres terminales distinctes s'ouvrent à l'écran sans se superposer totalement (Commande, Aiguillages, ShuttleManager, et les 4 Robots).
4. Stoppez le lancement (Ctrl+C), puis relancez avec `nbRobot:=2`. Vérifiez que seules 5 fenêtres s'ouvrent, confirmant l'absence des terminaux pour les robots 3 et 4.
